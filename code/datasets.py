        ...
        for iPatient in np.arange(0, len(self.unique_patients)):
            ...
            for iSlice in np.arange(0, slices_per_volume):
                    ...
                    for iModality in np.arange(0, len(self.item)):
                        ...
                        # Normalization
                        x = image_normalization(x, self.input_shape, norm=self.norm, channels=self.nchannels,
                        ...
                        if 'malign' in slices_patient[iSlice]:
                            ...
                            # Normalization
                            y = image_normalization(y, self.input_shape, norm=self.norm, channels=1,
                                                    histogram_matching=False,
                                                    reference_image=None,
                                                    mask=True, channel_first=True)
                            self.M[iPatient, iSlice, :, :, :] = y
...
class WSALDataGenerator(object):
    ...
    def __init__(self, dataset, partition, batch_size=16, shuffle=False):
        ...
        if self.dataset.weak_supervision:
            self.indexes_abnormal = self.dataset.train_indexes_abnormal.copy()
            self._idx_abnormal = 0
            self.batch_size_anomaly = np.min(np.array((self.batch_size, len(self.dataset.train_indexes_abnormal))))
